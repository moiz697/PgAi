...
query = "SELECT * FROM  msci_pak_global_stock"
...
# Define callbacks
callbacks = [
    ModelCheckpoint(' msci_pak_global_stock.keras', save_best_only=True),
    TensorBoard(log_dir='./logs', histogram_freq=1),
    EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
]

# Training the model
history = model.fit(x, y, epochs=100, batch_size=10, verbose=1, validation_split=0.05, callbacks=callbacks)



# No need to call model.save(' msci_pak_global_stock.keras') if you're saving to PostgreSQL
...
model.save(' msci_pak_global_stock.keras')


# Load the saved model using Keras
loaded_model = load_model(' msci_pak_global_stock.keras')
...
model_keras = load_model(' msci_pak_global_stock.keras')

# Save the model in .h5 format
model_keras.save(' msci_pak_global_stock.h5')
