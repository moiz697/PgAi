...
#define GEN_PASS_DECL_LEGALIZESPARSEOPSPASS
...
//===----------------------------------------------------------------------===//
// LegalizeSparseOpsPass
//===----------------------------------------------------------------------===//
#ifdef GEN_PASS_DECL_LEGALIZESPARSEOPSPASS
struct LegalizeSparseOpsPassOptions {
  bool legalize_to_custom_calls_ = true;
};
#undef GEN_PASS_DECL_LEGALIZESPARSEOPSPASS
#endif // GEN_PASS_DECL_LEGALIZESPARSEOPSPASS
#ifdef GEN_PASS_DEF_LEGALIZESPARSEOPSPASS
namespace impl {

template <typename DerivedT>
class LegalizeSparseOpsPassBase : public ::mlir::OperationPass<func::FuncOp> {
public:
  using Base = LegalizeSparseOpsPassBase;

  LegalizeSparseOpsPassBase() : ::mlir::OperationPass<func::FuncOp>(::mlir::TypeID::get<DerivedT>()) {}
  LegalizeSparseOpsPassBase(const LegalizeSparseOpsPassBase &other) : ::mlir::OperationPass<func::FuncOp>(other) {}

  /// Returns the command-line argument attached to this pass.
  static constexpr ::llvm::StringLiteral getArgumentName() {
    return ::llvm::StringLiteral("legalize-sparse-ops");
  }
  ::llvm::StringRef getArgument() const override { return "legalize-sparse-ops"; }

  ::llvm::StringRef getDescription() const override { return "Legalize from sparse ops before convert MLIR to XLA computation."; }

  /// Returns the derived pass name.
  static constexpr ::llvm::StringLiteral getPassName() {
    return ::llvm::StringLiteral("LegalizeSparseOpsPass");
  }
  ::llvm::StringRef getName() const override { return "LegalizeSparseOpsPass"; }
  ...
  /// instantiation because Pass classes should only be visible by the current
  /// library.
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(LegalizeSparseOpsPassBase<DerivedT>)

  LegalizeSparseOpsPassBase(const LegalizeSparseOpsPassOptions &options) : LegalizeSparseOpsPassBase() {
    legalize_to_custom_calls_ = options.legalize_to_custom_calls_;
  }
protected:
  ::mlir::Pass::Option<bool> legalize_to_custom_calls_{*this, "legalize-to-custom-calls", ::llvm::cl::desc("Whether legalize the sparse operations to custom_calls to be able to translate sparse operations to XLA computations"), ::llvm::cl::init(true)};
private:
};
} // namespace impl
#undef GEN_PASS_DEF_LEGALIZESPARSEOPSPASS
#endif // GEN_PASS_DEF_LEGALIZESPARSEOPSPASS
...
//===----------------------------------------------------------------------===//
// LegalizeSparseOpsPass Registration
//===----------------------------------------------------------------------===//

inline void registerLegalizeSparseOpsPass() {
  ::mlir::registerPass([]() -> std::unique_ptr<::mlir::Pass> {
    return createLegalizeSparseOperationsPass();
  });
}

// Old registration code, kept for temporary backwards compatibility.
inline void registerLegalizeSparseOpsPassPass() {
  ::mlir::registerPass([]() -> std::unique_ptr<::mlir::Pass> {
    return createLegalizeSparseOperationsPass();
  });
}
